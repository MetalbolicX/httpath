    ...
    withAuthFile([buildAuthLine("alice", "secret")], (authPath, tmpDir) => {
      ...
      (async () => {
        try {
          ...
          // Verify all rejection codes appear: 200 (5x), 401 (5x), 429 (1x), 405 (1x)
          const statusCounts = {};
          for (const line of logLines) {
          ...
          assert.strictEqual(statusCounts["429"], 1, `Expected 1x 429, got ${statusCounts["429"]}`);
          assert.strictEqual(statusCounts["405"], 1, `Expected 1x 405, got ${statusCounts["405"]}`);
        ...
        if (child.exitCode === null) child.kill("SIGTERM");
        rejectMain(e); // Reject so node:test registers the failure
      });
    });
  });
});
