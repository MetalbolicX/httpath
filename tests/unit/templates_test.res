...
test("Templates.escapeHtml: ampersand becomes &amp;", () => {
  ...
  assertion(~message="ampersand escaped", ~operator="=",
    (a, b) => a == b, result, "a &amp; b")
})

test("Templates.escapeHtml: less-than becomes &lt;", () => {
  let result = Templates.escapeHtml("<script>")
  assertion(~message="< escaped", ~operator="=",
    (a, b) => a == b, result, "&lt;script&gt;")
})
...
test("Templates.escapeHtml: double-quote becomes &quot;", () => {
  ...
  assertion(~message="double-quote escaped", ~operator="=",
    (a, b) => a == b, result, "say &quot;hello&quot;")
})
...
test("Templates.fileEntry: can construct with name, isDirectory, url", () => {
  ...
  assertion(~message="name field", ~operator="=",
    (a, b) => a == b, entry.name, "example.txt")
  assertion(~message="isDirectory false", ~operator="=",
    (a, b) => a == b, entry.isDirectory, false)
  assertion(~message="url field", ~operator="=",
    (a, b) => a == b, entry.url, "/example.txt")
})
...
test("renderDirectoryListing: non-root shows parent link", () => {
  ...
  // Should contain .. link
  assertion(~message="contains .. link",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "href=\"../\""),
    true)
})
...
test("renderDirectoryListing: root hides parent link", () => {
  ...
  assertion(~message="root has no parent link",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "href=\"../\""),
    false)
...
test("renderDirectoryListing: empty directory shows empty state", () => {
  ...
  assertion(~message="contains empty state text",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "This directory is empty"),
    true)
})
...
test("renderDirectoryListing: directory emoji for folders", () => {
  ...
  assertion(~message="contains folder emoji",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "📁"),
    true)
})
...
test("renderDirectoryListing: file emoji for files", () => {
  ...
  assertion(~message="contains file emoji",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "📄"),
    true)
...
test("renderDirectoryListing: directories sorted before files", () => {
  ...
  let gammaPos = Js.String2.indexOf(html, gammaAnchor)
  let alphaPos = Js.String2.indexOf(html, alphaAnchor)
  let betaPos = Js.String2.indexOf(html, betaAnchor)
  // Both dirs (gamma, alpha) should appear before beta.txt (files)
  // gamma (dir) position should be less than beta (file) position
  assertion(~message="gamma before beta",
    ~operator="=",
    (a, b) => a < b,
    gammaPos,
    betaPos)
  assertion(~message="alpha before beta",
    ~operator="=",
    (a, b) => a < b,
    alphaPos,
    betaPos)
...
test("renderDirectoryListing: alpha sorting within same type", () => {
  ...
  let alphaPos = Js.String2.indexOf(html, alphaAnchor)
  let betaPos = Js.String2.indexOf(html, betaAnchor)
  let zebraPos = Js.String2.indexOf(html, zebraAnchor)
  assertion(~message="alpha before beta",
    ~operator="=",
    (a, b) => a < b,
    alphaPos,
    betaPos)
  assertion(~message="beta before zebra",
    ~operator="=",
    (a, b) => a < b,
    betaPos,
    zebraPos)
...
test("renderDirectoryListing: title contains Index of", () => {
  ...
  assertion(~message="title has Index of",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "<title>Index of /documents</title>"),
    true)
})
...
test("renderDirectoryListing: urlPath is escaped in title", () => {
  ...
  assertion(~message="script tag escaped in title",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "&lt;script&gt;"),
    true)
})
...
test("renderDirectoryListing: file names are escaped", () => {
  ...
  assertion(~message="filename escaped",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "&lt;test&gt;"),
    true)
})
...
test("renderDirectoryListing: non-root URL is used as href", () => {
  ...
  assertion(~message="href contains url",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(html, "href=\"/dir/my%20file.txt\""),
    true)
})
