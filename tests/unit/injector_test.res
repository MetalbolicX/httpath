...
test("Injector.liveReloadScript: contains ws: protocol for http", () => {
  ...
  assertion(~message="contains ws:",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "ws:"),
    true)
})

test("Injector.liveReloadScript: contains wss: protocol for https", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="contains wss:",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "wss:"),
    true)
})

test("Injector.liveReloadScript: contains location.protocol check", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="checks location.protocol",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "window.location.protocol"),
    true)
})

test("Injector.liveReloadScript: contains the livereload endpoint path", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="contains /livereload",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, Types.liveReloadEndpoint),
    true)
})

test("Injector.liveReloadScript: contains the liveReloadMessage check", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="contains reload message check",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "if (event.data === '" ++ Types.liveReloadMessage ++ "')"),
    true)
})

test("Injector.liveReloadScript: contains reload() call", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="contains window.location.reload",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "window.location.reload()"),
    true)
})

test("Injector.liveReloadScript: contains 1000ms reconnect delay", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="contains 1000ms timeout",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "setTimeout(connect, 1000)"),
    true)
})

test("Injector.liveReloadScript: is wrapped in IIFE", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="starts with (() =>",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "(() => {"),
    true)
  assertion(~message="ends with })();",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "})();"),
    true)
...
test("Injector.liveReloadScript: wrapped in script tag", () => {
  let script = Injector.liveReloadScript(~port=5173)
  assertion(~message="starts with <script>",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "<script>"),
    true)
  assertion(~message="ends with </script>",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(script, "</script>"),
    true)
...
test("injectLiveReloadScript: inserts before </body> when present", () => {
  ...
  assertion(~message="contains </body>",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(result, "</body>"),
    true)
  // Script should appear before </body>
  let scriptPos = Js.String2.indexOf(result, "<script>")
  let bodyPos = Js.String2.indexOf(result, "</body>")
  assertion(~message="script before </body>",
    ~operator="=",
    (a, b) => a < b,
    scriptPos,
    bodyPos)
  // Count occurrences of <script> — should be exactly 1
  let parts = Js.String2.split(result, "<script>")
  let count = Array.length(parts) - 1
  assertion(~message="exactly one script tag",
    ~operator="=",
    (a, b) => a == b,
    count,
    1)
...
test("injectLiveReloadScript: inserts before </html> when no </body>", () => {
  ...
  assertion(~message="contains </html>",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(result, "</html>"),
    true)
  // Script should appear before </html>
  let scriptPos = Js.String2.indexOf(result, "<script>")
  let htmlEndPos = Js.String2.indexOf(result, "</html>")
  assertion(~message="script before </html>",
    ~operator="=",
    (a, b) => a < b,
    scriptPos,
    htmlEndPos)
...
test("injectLiveReloadScript: appends when neither </body> nor </html>", () => {
  ...
  assertion(~message="contains original p tag",
    ~operator="=",
    (a, b) => a == b,
    Js.String2.includes(result, "<p>Hello</p>"),
    true)
  // Script should appear at or after the end of the original HTML content
  let pEnd = Js.String2.indexOf(result, "</p>") + 5 // after </p>
  let scriptPos = Js.String2.indexOf(result, "<script>")
  assertion(~message="script at or after original content",
    ~operator="=",
    (a, b) => a >= b,
    scriptPos,
    pEnd)
...
test("injectLiveReloadScript: does single replacement only", () => {
  ...
  let parts = Js.String2.split(result, "</body>")
  let count = Array.length(parts) - 1
  // Should have exactly 2 </body> (replace only replaces first occurrence)
  assertion(~message="exactly 2 body tags",
    ~operator="=",
    (a, b) => a == b,
    count,
    2)
